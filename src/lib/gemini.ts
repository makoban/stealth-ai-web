...
export async function detectProperNouns(
  ...
  // 専門家: 本当に専門的な用語のみ
  const levelCriteria = {
    elementary: '小学生が知らない可能性がある単語を幅広く抽出。固有名詞、カタカナ語、専門用語、企業名、人名、地名など、小学校の教科書に出てこないものはすべて抽出',
    middle: '中学生が知らない可能性がある単語を抽出。中学校の教科書や一般的なニュースで見ない専門用語、業界用語、マイナーな固有名詞など',
    high: '高校生が知らない可能性がある単語を抽出。高校の教科書や一般常識を超える専門用語、業界固有の用語、最新のトレンド用語など',
    university: '大学生が知らない可能性がある単語を抽出。大学の一般教養を超える高度な専門用語、特定分野の専門知識が必要な用語のみ',
    expert: 'その分野の専門家でも調べる必要があるレベルの用語のみを抽出。最先端の研究用語、業界内でも一部の人しか知らないニッチな用語、新しく登場した概念や技術名のみ。一般的な地名・企業名・人名・製品名は完全に除外',
  };
...
export async function detectProperNounsExtended(
  ...
  const levelExclusions = {
    elementary: '※除外なし。小学生が知らない可能性があればすべて抽出',
    middle: '※以下は除外: 小学校で習う基本的な地名・国名のみ',
    high: '※以下は除外: 中学校までに習う地名・国名・歴史上の人物',
    university: '※以下は除外: 高校までに習う一般常識、有名企業・有名人の基本情報',
    expert: '※以下は完全に除外（専門家なら当然知っている）: すべての地名・国名、すべての有名企業・ブランド、すべての有名人、一般的な専門用語、業界で広く使われている用語。「その分野の専門家でも調べないとわからない」レベルの用語のみ抽出',
  };
