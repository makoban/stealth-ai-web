...
// ===========================================
const POINT_COSTS = {
  whisper: 1,    // Whisper API: 1ポイント/リクエスト
  gemini: 1,     // Gemini API: 1ポイント/リクエスト
  assemblyai: 1, // AssemblyAI: 1ポイント/リクエスト
};
...
// ポイントを消費
async function consumePoints(userId, apiType, amount) {
  ...
  try {
    ...
    // 履歴を記録
    await pool.query(
      `INSERT INTO stealth_point_history (user_id, change_amount, balance_after, reason, api_type)
       VALUES ($1, $2, $3, $4, $5)`,
      [userId, -amount, newBalance, 'api_usage', apiType]
    );
...
// ポイントチェックミドルウェア
// ===========================================
const checkPoints = (apiType) => async (req, res, next) => {
  // 認証されていない場合はスキップ（開発用）
  if (!req.user || !pool) {
    req.dbUser = null;
    return next();
  ...
  try {
    ...
    // ポイント残高チェック
    const cost = POINT_COSTS[apiType] || 1;
    if (dbUser.points < cost) {
      return res.status(402).json({ 
        error: 'Insufficient points',
        required: cost,
        remaining: dbUser.points,
      });
    }
...
// ===========================================
app.post('/api/whisper', upload.single('file'), checkPoints('whisper'), async (req, res) => {
  ...
  try {
    // ポイント消費（認証済みユーザーのみ）
    if (req.dbUser) {
      const pointResult = await consumePoints(req.dbUser.id, 'whisper', req.pointCost);
...
// ===========================================
app.post('/api/gemini', checkPoints('gemini'), async (req, res) => {
  const startTime = Date.now();
  ...
  try {
    // ポイント消費（認証済みユーザーのみ）
    if (req.dbUser) {
      const pointResult = await consumePoints(req.dbUser.id, 'gemini', req.pointCost);
      if (!pointResult.success) {
        return res.status(402).json({ 
          error: pointResult.error,
          remaining: pointResult.remaining,
...
// ===========================================
app.get('/api/assemblyai/token', checkPoints('assemblyai'), async (req, res) => {
  ...
  try {
    // ポイント消費（認証済みユーザーのみ）
    if (req.dbUser) {
      const pointResult = await consumePoints(req.dbUser.id, 'assemblyai', req.pointCost);
      if (!pointResult.success) {
        return res.status(402).json({ 
          error: pointResult.error,
          remaining: pointResult.remaining,
